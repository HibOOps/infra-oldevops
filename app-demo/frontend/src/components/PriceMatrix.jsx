        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
          <thead>
            <tr style={{ background: '#f8fafc' }}>
              <th className="table-sticky-col" style={thStyle}>SKU</th>
              <th style={thStyle}>Produit</th>
              <th className="table-col-hide-mobile" style={thStyle}>Catégorie</th>
              <th className="table-col-hide-mobile" style={thStyle}>Réf.</th>
              {channels.map(ch => (
          ...
          <tbody>
            {products.map((product, i) => {
              ...
              return (
                <tr key={product.id} style={{ background: rowBg, borderBottom: '1px solid #f1f5f9' }}>
                  <td className="table-sticky-col" style={{ ...tdStyle, background: rowBg }}>
                    <code style={{ fontSize: '0.75rem', color: '#6366f1' }}>{product.sku}</code>
                  </td>
                  <td style={{ ...tdStyle, maxWidth: '180px' }}>{product.name}</td>
                  <td className="table-col-hide-mobile" style={tdStyle}>
                    <span style={{ fontSize: '0.75rem', background: '#f1f5f9', borderRadius: '4px', padding: '2px 8px' }}>{product.category}</span>
                  </td>
                  <td className="table-col-hide-mobile" style={{ ...tdStyle, fontWeight: 600 }}>
                    {parseFloat(product.referencePrice).toFixed(2)} €
                  </td>
                  {channels.map(ch => {
                        ...
                        {priceObj ? (
                          isEditing ? (
                            <input
                              ...
                              style={{ width: '80px', minHeight: '44px', padding: '4px', borderRadius: '4px', border: '1px solid #6366f1', textAlign: 'center', fontSize: '1rem' }}
                            />
                          ) : (
                            <span
