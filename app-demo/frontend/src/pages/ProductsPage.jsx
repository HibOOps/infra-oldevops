...
export default function ProductsPage({ token }) {
  ...
  const inputStyle = { width: '100%', padding: '8px 10px', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '1rem', boxSizing: 'border-box', minHeight: '44px' }
        ...
        <div style={{ background: '#fff', borderRadius: '12px', padding: '24px', boxShadow: '0 2px 12px rgba(0,0,0,0.1)', marginBottom: '24px' }}>
          ...
          <form onSubmit={handleSubmit} className="form-grid-2col">
            ...
            <div>
              ...
              <input style={inputStyle} type="number" step="0.01" inputMode="decimal" value={form.referencePrice} onChange={e => setForm(f => ({ ...f, referencePrice: e.target.value }))} required />
            </div>
                ...
                <thead>
                  <tr style={{ background: '#f8fafc' }}>
                    {['SKU', 'Nom', 'Catégorie', 'Prix réf.', 'Statut sync', 'Actions'].map(h => (
                      <th
                        key={h}
                        className={h === 'Catégorie' ? 'table-col-hide-mobile' : undefined}
                  ...
                  {products.map((p, i) => (
                    <tr key={p.id} style={{ background: i % 2 === 0 ? '#fff' : '#f8fafc', borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <td className="table-col-hide-mobile" style={{ padding: '10px 14px' }}>
                        <span style={{ fontSize: '0.75rem', background: '#f1f5f9', borderRadius: '4px', padding: '2px 8px' }}>{p.category}</span>
                      </td>
                      ...
                      <td style={{ padding: '10px 14px' }}>
                        <div className="btn-group">
                          <button onClick={() => openEdit(p)} className="btn-action" style={{ padding: '4px 12px', borderRadius: '6px', border: '1px solid #cbd5e1', background: '#fff', cursor: 'pointer', fontSize: '0.8rem' }}>Éditer</button>
                          <button onClick={() => handleDelete(p.id)} className="btn-action" style={{ padding: '4px 12px', borderRadius: '6px', border: '1px solid #fca5a5', background: '#fef2f2', color: '#dc2626', cursor: 'pointer', fontSize: '0.8rem' }}>Supprimer</button>
                        </div>
                      </td>
                    </tr>
                  ))}
